...
--
-- Client :  localhost
-- Généré le :  Mer 24 Février 2016 à 15:11
...
--
-- Structure de la table `services_have_data_servers`
--

CREATE TABLE IF NOT EXISTS `services_have_data_servers` (
  `service_reference` varchar(50) NOT NULL,
...
ALTER TABLE `regular_timetable`
  ...
  ADD KEY `regular_timetable_fk_config_reference` (`config_reference`);
...
ALTER TABLE `services`
  ADD PRIMARY KEY (`reference`),
  ADD KEY `services_fk_service_type_reference` (`service_type_reference`),
  ADD KEY `services_fk_calc_server_reference` (`calc_server_reference`);

--
-- Index pour la table `services_have_data_servers`
--
ALTER TABLE `services_have_data_servers`
  ADD PRIMARY KEY (`service_reference`,`server_reference`),
  ADD KEY `services_for_data_servers_fk_server_reference` (`server_reference`);
...
--
ALTER TABLE `regular_timetable`
  ADD CONSTRAINT `regular_timetable_fk_config_reference` FOREIGN KEY (`config_reference`) REFERENCES `configurations` (`reference`),
...
ALTER TABLE `services`
  ...
  ADD CONSTRAINT `services_fk_service_type_reference` FOREIGN KEY (`service_type_reference`) REFERENCES `services_types` (`reference`);

--
-- Contraintes pour la table `services_have_data_servers`
--
ALTER TABLE `services_have_data_servers`
  ADD CONSTRAINT `services_for_data_servers_fk_server_reference` FOREIGN KEY (`server_reference`) REFERENCES `servers` (`reference`),
  ADD CONSTRAINT `services_for_data_servers_fk_service_reference` FOREIGN KEY (`service_reference`) REFERENCES `services` (`reference`);
